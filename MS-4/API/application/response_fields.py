...
reply_output_with_response_fields = {
    ...
    "files":fields.List(fields.Nested(file_output_with_response_fields), attribute=lambda obj: db.session.query(TicketFiles).filter_by(TicketFiles.attached_to == obj.id).order_by(TicketFiles.created_at.desc()).all()),
    "votes":{
...
ticket_output_with_response_fields = {
    ...
    "files":fields.List(fields.Nested(file_output_with_response_fields), attribute=lambda obj: db.session.query(TicketFiles).filter_by(TicketFiles.attached_to == obj.id).order_by(TicketFiles.created_at.desc()).all()),
    "replies":fields.List(fields.Nested(reply_output_with_response_fields), attribute=lambda obj: obj.replies.order_by(TicketReplies.created_at.desc()).all()),
    "votes":{
